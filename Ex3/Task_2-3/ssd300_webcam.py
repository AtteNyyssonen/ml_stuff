...
while True:
    ...
    ret, img = cam.read()
    height_orig, width_orig, channels = img.shape
    img = cv2.flip(img, 1)
    img_copy = imresize(img, (img_height, img_width), preserve_range=True)
    img_array = np.array([img_copy])

    pred_data = model.predict(img_array)
    
    width_ratio = width_orig / img_width
    height_ratio = height_orig / img_height

    for prediction in pred_data[0]:
        ...
        if confidence > confidence_threshold:
            ...
            cv2.putText(img, f"{classes[int(class_id)]}", (xmin, ymin - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    ##-your-code-ends-here-##
