...
for i1 in range(npts1):
    for i2 in range(npts2):
        numerator = np.sum((patches1[:,:,i1]-np.mean(patches1[:,:,i1]))*(patches2[:,:,i2]-np.mean(patches2[:,:,i2])))
        denominator = np.sqrt(np.sum((patches1[:,:,i1]-np.mean(patches1[:,:,i1]))**2)) * np.sqrt(np.sum((patches2[:,:,i2]-np.mean(patches2[:,:,i2]))**2))
