...
# 2 matches, the front of the boat and the big house above the detection at the boats front seem
# to be correct. The others are off by varying accuracy.
...
for i, index in enumerate(pairs[:,0]):
    nearest = distmat_sorted[int(index), 0]
    second_nearest = distmat_sorted[int(index), 1]
...
# How many of the top 5 matches appear to be correct correspondences?
# All 5 matches seem correct. The boats front, the window and the building above the front of the boat are matched
# correctly. The matching also happens to what appears to be a door at the larger boat and then the larger vicinity
# of the door. These two are also matched correctly between the images.